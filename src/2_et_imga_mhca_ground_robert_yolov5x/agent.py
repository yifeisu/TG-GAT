...
class NavCMTAgent:
    ...
    def rollout(self, train_ml=None, not_in_train=False, nss_w=0, **kwargs):
        ...
        input = {
            ...
            'frames': torch.zeros(batch_size, 0, 1280, 49).cuda(),
        ...
        for t in range(self.args.max_action_len):
            ...
            # [bs,1280,49]
            im_feature = im_feature.flatten(-2, -1)
            ...
            input['frames'] = torch.hstack((input['frames'], im_feature.view(-1, 1, 1280, 49)))
            ...
            if not 'test' in self.env_name:
                ...
                ml_loss += 0.2 * (1 * loss_cls + 3 * loss_f1 + 1.5 * loss_iou)
                if ml_loss != ml_loss:  # debug for nan loss
                    print('box', ml_loss)
