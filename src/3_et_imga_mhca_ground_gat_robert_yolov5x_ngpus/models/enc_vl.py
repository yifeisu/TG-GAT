...
class EncoderVL(nn.Module):
    ...
    def forward(
        ...
        # mask_attn: [len, len], len=language_len+vision_len+actin_len
        # [len, len] -> [bs*n_head,len,len]
        mask_attn = mask_attn.unsqueeze(0).repeat(emb_lang.shape[0] * self.args.encoder_heads, 1, 1)
        # frames_graph: [bs,len,len] -> [bs*n_head,vision_len,vision_len]
        frames_graph = frames_graph.repeat(self.args.encoder_heads, 1, 1)
        # merge
        mask_attn[:, length_lang:length_lang + length_max, length_lang:length_lang + length_max] = \
            mask_attn[:, length_lang:length_lang + length_max, length_lang:length_lang + length_max] + frames_graph
